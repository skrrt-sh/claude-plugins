...
3. Choose the commit `type` from `reference/commit-types.md`.
4. Choose the optional `scope` from the dominant subsystem, package, app, directory, or concern.
5. Choose the best `gitmoji` from `reference/gitmojis.md`.
6. Stage only the intended files or hunks.
7. Write the commit header in the required repository format.
8. Write a description body explaining what changed and why. Treat the body as required for this skill.
9. Add footer lines for breaking changes, issue references, or follow-up metadata.
10. Commit with `git`.
